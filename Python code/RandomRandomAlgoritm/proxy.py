    ...
    try:
        ...
        if selected_datetime_from and selected_datetime_to:
            ...
            if end_date_timestamp >= hard_coded_last_api_entry_date:
                #last_data_timestamp = data[-1]['timestamp']
                #last_data_timestamp_trimmed = trimDateStr(last_data_timestamp)
                #print("last_data_timestamp", last_data_timestamp_trimmed)
                ...
                no_gen_mmissing_ts = len(generated_missing_timestamps)
                print("Generated_missing_timestamps", no_gen_mmissing_ts, " št. gen. elementov ", 3*no_gen_mmissing_ts)
                ...
                for index, timestamp in enumerate(generated_missing_timestamps):
                    for id in ['96', '97', '98']:
                        ...
                        current_predictive_data = []
                        # Lokaliziramo timestamp vrednost, ker se dogaja race condition v python kodi, ki se ga ne znam rešit
                        #lokalni_timestamp = str(timestamp)
                        lokalni_timestamp = str(generated_missing_timestamps[index])
                        ...
                        if matching_entry is None:
                            ...
                            if future_timestamp is None or are_timestamps_equal(timestamp, future_timestamp):
                                #This adds for ID 96
                                current_predictive_data_ID96 = generate_predictive_data(id, lokalni_timestamp)
                                current_predictive_data = current_predictive_data_ID96
                                #data.append(current_predictive_data_ID96)
                                print("current_predictive_data_ID96['timestamp']", current_predictive_data_ID96['timestamp'])
                                ...
                                future_timestamp = add_minutes_to_timestamp(lokalni_timestamp, additional_minutes)
                                #else:
                                # Add generate data for ID 97 and ID 98
                            if id == '97' and current_predictive_data_ID97 == None:
                                current_predictive_data_ID97 = generate_predictive_data(id, lokalni_timestamp)
                                print("current_predictive_data_ID97['timestamp']", current_predictive_data_ID97['timestamp'])
                                current_predictive_data = current_predictive_data_ID97
                                #data.append(current_predictive_data_ID97)
                            if id == '98' and current_predictive_data_ID98 == None:
                                current_predictive_data_ID98 = generate_predictive_data(id, lokalni_timestamp)
                                print("current_predictive_data_ID98['timestamp']", current_predictive_data_ID98['timestamp'])
                            ...
                            if id == '96' and current_predictive_data is None:
                                current_predictive_data_ID96['timestamp'] = lokalni_timestamp
                                print("For_LOOP current_predictive_data_ID96['timestamp']", current_predictive_data_ID96['timestamp'])
                                current_predictive_data = current_predictive_data_ID96
                                #data.append(current_predictive_data_ID96)
                            if id == '97' and current_predictive_data is None:
                                current_predictive_data_ID97['timestamp'] = lokalni_timestamp
                                #print("current_predictive_data_ID97['timestamp']", current_predictive_data_ID97['timestamp'])
                                current_predictive_data = current_predictive_data_ID97
                                #data.append(current_predictive_data_ID97)
                            if id == '98' and current_predictive_data is None:
                                current_predictive_data_ID98['timestamp'] = lokalni_timestamp
                                #print("current_predictive_data_ID98['timestamp']", current_predictive_data_ID98['timestamp'])
                                current_predictive_data = current_predictive_data_ID98
                                #data.append(current_predictive_data_ID98)
                            temp_predictive_data = list(current_predictive_data)
                            data.append(temp_predictive_data)

        #print("Število zapisov v data", len(gendata))
        #data.append(gendata)
